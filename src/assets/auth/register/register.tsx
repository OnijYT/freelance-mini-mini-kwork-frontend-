import { useContext, useState } from 'react'
import s from './Register.module.css'
import { api } from '../../../api'
import { AuthContext } from '../../../context/AuthContext'
import { useNavigate } from 'react-router-dom'
import axios from 'axios'
import type { ApiResponse, User } from '../../../types/auth'

function Register() {
    const [showPassword, setShowPassword] = useState(false)
    const [showConfirmPassword, setShowConfirmPassword] = useState(false)
    const [isLoading, setIsLoading] = useState(false)
    const [acceptTerms, setAcceptTerms] = useState(false)
    const [confirmpass, setConfirmpass] = useState('')

    const [email, setEmail] = useState<string>('')
    const [password, setPassword] = useState<string>('')
    const [fullname, setFullname] = useState<string>('')
    const [role, setRole] = useState<string>('')
    const auth = useContext(AuthContext)
    const navigate = useNavigate()

    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault()
        setIsLoading(true)
        if(password !== confirmpass) {
                setIsLoading(false)
                return alert('Пароль не совпадает')
            }
        try {
            const res = await api.post<ApiResponse<User>>('/auth/register', {
                email,
                password,
                fullname,
                role
            })

            
            if(res.data) {
                auth?.login(res.data)
                navigate('/')
            } else{
                navigate('/login')
            }
            alert('Успешная регистрация')

            setIsLoading(false)
        }catch(err) {
            console.error(err)
            setIsLoading(false)
            if(axios.isAxiosError(err)) {
                const message = err.response?.data?.message || 'Ошибка сервера'
                alert(message)
            }else {
                alert("Произошла ошибка")
            }
        }
    }

    return (
        <div className={s.registerContainer}>
            <div className={s.registerCard}>
                <div className={s.logoContainer}>
                    <div className={s.logo}>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" />
                        </svg>
                    </div>
                </div>

                <div className={s.registerHeader}>
                    <h1 className={s.title}>Создать аккаунт</h1>
                    <p className={s.subtitle}>Присоединяйтесь к нашей платформе</p>
                </div>

                <form className={s.registerForm} onSubmit={handleSubmit}>
                    <div className={s.inputGroup}>
                        <label htmlFor="name" className={s.label}>
                            Полное имя
                        </label>
                        <div className={s.inputWrapper}>
                            <div className={s.inputIconWrapper}>
                                <svg className={s.inputIcon} viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                </svg>
                            </div>
                            <input
                                id="name"
                                name="name"
                                type="text"
                                className={s.input}
                                placeholder="Иван Иванов"
                                value={fullname}
                                onChange={(e) => setFullname(e.target.value)}
                                required
                            />
                        </div>
                    </div>

                    <div className={s.inputGroup}>
                        <label htmlFor="email" className={s.label}>
                            Email адрес
                        </label>
                        <div className={s.inputWrapper}>
                            <div className={s.inputIconWrapper}>
                                <svg className={s.inputIcon} viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207" />
                                </svg>
                            </div>
                            <input
                                id="email"
                                name="email"
                                type="email"
                                className={s.input}
                                placeholder="example@mail.com"
                                value={email}
                                onChange={(e) => setEmail(e.target.value)}
                                required
                            />
                        </div>
                    </div>

                    <div className={s.roleSelection}>
                        <label className={s.label}>
                            Выберите роль
                        </label>
                        <div className={s.roleOptions}>
                            <label className={`${s.roleCard} ${role === 'freelancer' ? s.roleCardActive : ''}`}>
                                <input
                                    type="radio"
                                    name="role"
                                    value="freelancer"
                                    checked={role === 'freelancer'}
                                    onChange={(e) => setRole(e.target.value)}
                                    className={s.roleInput}
                                    required
                                />
                                <div className={s.roleIcon}>
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                                    </svg>
                                </div>
                                <div className={s.roleContent}>
                                    <h3 className={s.roleTitle}>Фрилансер</h3>
                                    <p className={s.roleDescription}>Предлагайте свои услуги и зарабатывайте</p>
                                </div>
                                <div className={s.roleCheck}>
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                                    </svg>
                                </div>
                            </label>

                            <label className={`${s.roleCard} ${role === 'client' ? s.roleCardActive : ''}`}>
                                <input
                                    type="radio"
                                    name="role"
                                    value="client"
                                    checked={role === 'client'}
                                    onChange={(e) => setRole(e.target.value)}
                                    className={s.roleInput}
                                    required
                                />
                                <div className={s.roleIcon}>
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                    </svg>
                                </div>
                                <div className={s.roleContent}>
                                    <h3 className={s.roleTitle}>Клиент</h3>
                                    <p className={s.roleDescription}>Находите исполнителей для ваших задач</p>
                                </div>
                                <div className={s.roleCheck}>
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                                    </svg>
                                </div>
                            </label>
                        </div>
                    </div>

                    <div className={s.inputRow}>
                        <div className={s.inputGroup}>
                            <label htmlFor="password" className={s.label}>
                                Пароль
                            </label>
                            <div className={s.inputWrapper}>
                                <div className={s.inputIconWrapper}>
                                    <svg className={s.inputIcon} viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                                    </svg>
                                </div>
                                <input
                                    id="password"
                                    name="password"
                                    type={showPassword ? 'text' : 'password'}
                                    className={s.input}
                                    placeholder="••••••••"
                                    value={password}
                                    onChange={(e) => setPassword(e.target.value)}
                                    required
                                />
                                <button
                                    type="button"
                                    className={s.togglePassword}
                                    onClick={() => setShowPassword(!showPassword)}
                                >
                                    {showPassword ? (
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" />
                                        </svg>
                                    ) : (
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                        </svg>
                                    )}
                                </button>
                            </div>
                        </div>

                        <div className={s.inputGroup}>
                            <label htmlFor="confirmPassword" className={s.label}>
                                Подтвердите пароль
                            </label>
                            <div className={s.inputWrapper}>
                                <div className={s.inputIconWrapper}>
                                    <svg className={s.inputIcon} viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                                    </svg>
                                </div>
                                <input
                                    id="confirmPassword"
                                    name="confirmPassword"
                                    type={showConfirmPassword ? 'text' : 'password'}
                                    className={s.input}
                                    placeholder="••••••••"
                                    value={confirmpass}
                                    onChange={(e) => setConfirmpass(e.target.value)}
                                    required
                                />
                                <button
                                    type="button"
                                    className={s.togglePassword}
                                    onClick={() => setShowConfirmPassword(!showConfirmPassword)}
                                >
                                    {showConfirmPassword ? (
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" />
                                        </svg>
                                    ) : (
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                        </svg>
                                    )}
                                </button>
                            </div>
                        </div>
                    </div>

                    <div className={s.termsSection}>
                        <label className={s.checkboxLabel}>
                            <input 
                                type="checkbox" 
                                className={s.checkbox}
                                checked={acceptTerms}
                                onChange={(e) => setAcceptTerms(e.target.checked)}
                                required
                            />
                            <span>
                                Я принимаю{' '}
                                <a href="#" className={s.link}>Условия использования</a>
                                {' '}и{' '}
                                <a href="#" className={s.link}>Политику конфиденциальности</a>
                            </span>
                        </label>
                    </div>

                    <button type="submit" className={s.submitButton} disabled={isLoading || !acceptTerms}>
                        {isLoading ? (
                            <span className={s.loader}></span>
                        ) : (
                            <>
                                <span>Создать аккаунт</span>
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" />
                                </svg>
                            </>
                        )}
                    </button>
                </form>

                <div className={s.divider}>
                    <span>или зарегистрироваться с</span>
                </div>

                <div className={s.socialButtons}>
                    <button className={`${s.socialButton} ${s.google}`}>
                        <svg viewBox="0 0 24 24" fill="currentColor">
                            <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                            <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                            <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
                            <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
                        </svg>
                        <span>Google</span>
                    </button>
                    <button className={`${s.socialButton} ${s.facebook}`}>
                        <svg viewBox="0 0 24 24" fill="currentColor">
                            <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                        </svg>
                        <span>Facebook</span>
                    </button>
                    <button className={`${s.socialButton} ${s.github}`}>
                        <svg viewBox="0 0 24 24" fill="currentColor">
                            <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                        </svg>
                        <span>GitHub</span>
                    </button>
                </div>

                <div className={s.footer}>
                    <p>
                        Уже есть аккаунт?{' '}
                        <a href="/login" className={s.loginLink}>
                            Войти
                        </a>
                    </p>
                </div>

                <div className={s.securityBadge}>
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                    </svg>
                    <span>Ваши данные защищены</span>
                </div>
            </div>

            <div className={s.backgroundDecoration}>
                <div className={s.circle1}></div>
                <div className={s.circle2}></div>
                <div className={s.circle3}></div>
                <div className={s.circle4}></div>
                <div className={s.floatingShape1}></div>
                <div className={s.floatingShape2}></div>
                <div className={s.floatingShape3}></div>
            </div>
        </div>
    )
}

export default Register
